...
const loginOs = async () => {
  ...
  try {
    loading.value = true;
    await authStore.dispatchLoginKrbManual(username.value, password.value);
...
const loginAuto = async () => {
  ...
  try {
    loading.value = true;
    await authStore.dispatchLoginKrbAuto();
    redirectToDesiredPage();
  } catch (error) {
    toast.error(t('Auth.notifications.incorrectAuth'));
  } finally {
...
<template>
  <div id="svyazcom-app" class="layout flex align-items-center justify-content-center">
    <SvCard class="layout-body pb-2">
      ...
      <SvField :label="$t('Common.fields.domain')" class="pb-4">
        <SvInputText v-model="domain" disabled />
      </SvField>
      <SvField :label="$t('Common.fields.login')" class="pb-4">
        <SvInputText v-model="username" />
      </SvField>
      ...
      <div class="flex justify-content-center pb-3">
        <SvButton class="w-full" :label="$t('Auth.buttons.signIn')" :loading="loading" @click="loginOs" />
      </div>
      <div class="flex justify-content-center pb-3">
        <SvButton text :label="$t('Auth.buttons.signInWithLocalAccount')" @click="toLogin" />
      </div>
    </SvCard>
  </div>
</template>
